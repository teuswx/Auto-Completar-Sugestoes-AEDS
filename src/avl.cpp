...
void insertItem_AVL(Tree_AVL **t, Record_AVL item) {
    if (*t == NULL) {
        *t = (Tree_AVL *)malloc(sizeof(Tree_AVL));
        ...
        // (*t)->item = item;
        (*t)->item.key = item.key;
        (*t)->item.value = (char *)malloc(sizeof(char) * 100);
        (*t)->peso = 0;
        strcpy((*t)->item.value, item.value);
        (*t)->item.values = (Vet *)malloc(sizeof(Vet) * 20);
        (*t)->item.i = 0;
        ...
        (*t)->item.values[(*t)->item.i].palavra = (char *)malloc(sizeof(char) * 100);
        strcpy((*t)->item.values[(*t)->item.i++].palavra, item.value);
    }
...
void preordem_AVL(Tree_AVL *t) {
    if (!(t == NULL)) {
        ...
        arquivo_saida << "[frequência: " << t->item.key << " valor: " << t->item.value << " ";
        for (int i = 0; i < t->item.i; i++) {
            arquivo_saida << t->item.values[i].palavra << " ";
        }
        arquivo_saida << "] ";
        preordem_AVL(t->esq);
        preordem_AVL(t->dir);
